...
const styles =  theme => ({
  ...
  drawerLabel: {
    textAlign: 'center',
    opacity:    '0',
    transition: 'opacity .1s, transition 1s cubic-bezier(0.49, 0.01, 0, 1)',
    transform:  'translateY(-200px)'
  },
  drawerLabelAnimate: {
    opacity:    '1',
    transition: 'all 1s cubic-bezier(0.49, 0.01, 0, 1)',
  ...
  ripples: {
    ...
    opacity:          '1',
    zIndex:           '2',
    backgroundImage: 'linear-gradient(  black 80%,  #e442fc 80%, #61dafb 84%, transparent 95%)'
  },
  explode: {
...
class AppToolbar extends Component {
  ...
  state = {
    ...
    // for AppShell to route to page
    const handleDrawerLinkClicked = async (label) => {
      const link = `/${label}`;
      this.props.onDrawerLinkClicked(link);
      handleDrawer();
    };
    ...
    const { classes, pages, theme } = this.props;
 		...
 		return (
          ...
          <div className={classes.drawerHeader}>
	          <IconButton style={{color: '#61dafb'}} onClick={handleDrawer}>
	            {<KeyboardArrowUpIcon />}
	          </IconButton>
	        </div>
	        <List id="list" className={classes.drawerList}>
	          {this.pages.map((page, index) => (
	             <ListItem button key={page.label} onClick={() => handleDrawerLinkClicked(page.label)}>
	              <ListItemText className={clsx(classes.drawerLabel, { [classes.drawerLabelAnimate]: this.state.open})} primary={page.label} />
	            </ListItem>
	          ))}
	        </List>
	      </Drawer>
        <div id="clickRipple" className={clsx(classes.ripples, { [classes.explode]: this.state.open})}></div>
	    </header>
